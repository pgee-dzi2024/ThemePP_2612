...
var options = {
    ...
    xaxis: {
        type: 'day',
        categories: ['Jan', 'Fab', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Spt',
            'Oct', 'Nov', 'Dec'
        ],
        axisBorder: {
            show: true,
...
document.addEventListener("DOMContentLoaded", function () {
  var chartContainer = document.querySelector("#candidates-chart");
  ...
  if (chartContainer) {
    ...
    // Make Candidates function accessible globally
    window.Candidates = function () {
      if (typeof myVarVal !== "undefined") {
        chart.updateOptions({
          colors: ["rgb(" + myVarVal + ")", "#23b7e5"],
        });
      } else {
